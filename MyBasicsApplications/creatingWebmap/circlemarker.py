...
for la,lo,el in zip(lat,lon,elev):
    ...
    fg.add_child(folium.CircleMarker(location=[la,lo],radius=10,fill=True,
    popup=str(el)+"m",fill_color=getColor(el),fill_opacity=1.0))
                                                     #folium.Popup(str(elev)+"m",parse_html=True)
map.add_child(fg)
map.save("Volcanoes_circles.html")
